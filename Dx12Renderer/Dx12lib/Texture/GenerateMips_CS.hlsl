...
void CS( ComputeShaderInput IN )
{
    ...
    switch ( SrcDimension )
    {
        case WIDTH_HEIGHT_EVEN:
        {
            ...
            Src1 = SampleTexture(SrcMip.SampleLevel(LinearClampSampler, UV, SrcMipLevel));
        }
        break;
        case WIDTH_ODD_HEIGHT_EVEN:
        {
            ...
            Src1 = 0.5 * ( SampleTexture(SrcMip.SampleLevel(LinearClampSampler, UV1,     SrcMipLevel)) +
                           SampleTexture(SrcMip.SampleLevel(LinearClampSampler, UV1+Off, SrcMipLevel)));
        }
        break;
        ...
        case WIDTH_HEIGHT_ODD:
        {
            ...
            Src1 =  SampleTexture(SrcMip.SampleLevel( LinearClampSampler, UV1, SrcMipLevel ));
            Src1 += SampleTexture(SrcMip.SampleLevel( LinearClampSampler, UV1 + float2( Off.x, 0.0   ), SrcMipLevel ));
            Src1 += SampleTexture(SrcMip.SampleLevel( LinearClampSampler, UV1 + float2( 0.0,   Off.y ), SrcMipLevel ));
            Src1 += SampleTexture(SrcMip.SampleLevel( LinearClampSampler, UV1 + float2( Off.x, Off.y ), SrcMipLevel ));
            Src1 *= 0.25;
        }
        break;
    }
