...
#define __PIXEL_PACKING_VELOCITY_HLSLI__
...
#if defined(PACKING_VELOCITY_UINT32) || defined(PACKING_VELOCITY_FLOAT32)
	...
	#else
		#define packed_velocity_t float
		packed_velocity_t PackVelocity(float3 Velocity)
		{
			return asfloat(PackXY(Velocity.x) | PackXY(Velocity.y) << 10 | PackZ(Velocity.z) << 20);
		}
		// Unpack the velocity from R10G10B10A2_UNORM
		float3 UnpackVelocity(packed_velocity_t Velocity)
		{
			uint v = asuint(Velocity);
			return float3(UnpackXY(v & 0x3FF), UnpackXY((v >> 10) & 0x3FF), UnpackZ(v >> 20));
		}
	#endif
