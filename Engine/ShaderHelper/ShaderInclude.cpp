...
{
    char *text;
    auto shaderFileName = std::filesystem::path(filename).filename();
    std::string finalPath = std::format("{}/{}", EUREKA_HLSL_SHADER_PATH, shaderFileName.string());
    auto fs = cmrc::Eureka::get_filesystem();
    if (fs.exists(finalPath)) {
        cmrc::file file = fs.open(finalPath);
        text = (char *)malloc(file.size() + 1);
        memcpy(text, file.begin(), file.size());
        text[file.size()] = '\0';
        return text;
    }
...
static std::string convertToAbsolute(const char *filename, const char *pathToInclude) {
    auto shaderFileName = std::filesystem::path(filename).filename();
    std::string finalPath = std::format("{}/{}", EUREKA_HLSL_SHADER_PATH, shaderFileName.string());
    auto fs = cmrc::Eureka::get_filesystem();
    ...
    // system path
    if (fs.exists(finalPath)) {
        finalPath = "Engine/" + finalPath;
        auto absolutePath = std::filesystem::canonical(std::filesystem::path(finalPath));
        return absolutePath.string();
    } else {
        std::string finalPath = std::format("{}/{}", pathToInclude, filename);
        auto absolutePath = std::filesystem::canonical(std::filesystem::path(finalPath));
        return absolutePath.string();
    }
    ...
    for (i = 0; i < num; ++i) {
...
#endif
        {
            ...
            if (inc_list[i].filename == 0)
                ...
                std::string path = convertToAbsolute(inc_list[i].filename, path_to_includes);
...
char *stb_include_file(char *filename, char *inject, char *path_to_includes, char error[256])
{
    ...
    auto absolutePath = std::filesystem::canonical(std::filesystem::path(filename));
    std::string newFilename = std::format("\"{}\"", absolutePath.string());
